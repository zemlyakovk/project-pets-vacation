              ...
              <div class="max-w-lg bg-white rounded-lg  modalAvatar">
                ...
                <div className="sitterInfo infoColor">
                  <div class="flex flex-col items-left pb-2 mr-8">
                    ...
                    {sitter.data.cat_flag ? <span class="mb-2 text-sm text-gray-500">Да</span> : <span class="mb-2 text-sm text-gray-500">Нет</span>}

                    <h5 class="mb-1 text-sm font-medium text-gray-900">Принимает собак</h5>
                    {sitter.data.dog_flag ? <span class="mb-2 text-sm text-gray-500">Да</span> : <span class="mb-2 text-sm text-gray-500">Нет</span>}

                    <h5 class="mb-1 text-gray-900 font-normal text-sm">Опыт в профессии </h5>
                    <span class="mb-2 text-sm text-gray-500">{sitter.data.experience} года/лет</span>

                    {/* <h5 class="mb-1 text-sm font-medium text-gray-900">Доступность</h5>
                  {sitter.data.active ? <span class="mb-2 text-sm text-gray-500">Да</span> : <span class="mb-2 text-sm text-gray-500">Нет</span>} */}

                  </div>
                  <div class="flex flex-col items-left pb-2 mr-8">
                    <h5 class="mb-1 text-sm font-medium text-gray-900">Домашние животные</h5>
                    {sitter.data.has_pet_flag ? <span class="mb-2 text-sm text-gray-500">Да</span> : <span class="mb-2 text-sm text-gray-500">Нет</span>}

                    <h5 class="mb-1 text-sm font-medium text-gray-900">Есть дети?</h5>
                    {sitter.data.has_child ? <span class="mb-2 text-sm text-gray-500">Да</span> : <span class="mb-2 text-sm text-gray-500">Нет</span>}

                    <h5 class="mb-1 text-sm font-medium text-gray-900">Готов смотреть:</h5>
                    {sitter.data.has_pet_flag ? <span class="mb-2 text-sm text-gray-500">Весь день</span> : <span class="mb-2 text-sm text-gray-500">Определенное время</span>}
                  ...
                  <div class="flex flex-col items-left pb-2 mr-8">
                    ...
                    {sitter.data.walking ? <span class="mb-2 text-sm text-gray-500">Да</span> : <span class="mb-2 text-sm text-gray-500">Нет</span>}

                    <div class="flex flex-col items-left pb-2 mr-8">
                      <h5 class="mb-1 text-sm font-medium text-gray-900">День передержки:</h5>
                      <span class="mb-2 text-sm text-gray-500">{sitter.data.price_per_day} рублей</span>

                      <h5 class="mb-1 text-sm font-medium text-gray-900">Час прогулки:</h5>
                      <span class="mb-2 text-sm text-gray-500">{sitter.data.price_per_hour} рублей</span>
                ...
                <div className="flex items-center p-6 space-x-2 rounded-b border-t border-gray-200 ">
                  ...
                  <Link className='' to={`/sitters/${id}`}>
                    <button data-modal-toggle="defaultModal" type="button" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Профиль ситтера</button>
                  </Link>
                </div>
